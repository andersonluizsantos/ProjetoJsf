...
public class SmartDateConverter implements Converter {
	...
	private Date getData(String dia) {
		Calendar dataHoje = Calendar.getInstance();
		if (dia == "amanha"){
			dataHoje.add(Calendar.DAY_OF_YEAR, +1);
		}
		if (dia == "ontem"){
			dataHoje.add(Calendar.DAY_OF_YEAR, -1);
		}
		//Zera as informações de hora, minuto, segundo e milisegundo
	...
	public Object getAsObject(FacesContext context, UIComponent component, String value) {
		...
		if (value != null && !value.equals("")) {
			if (value.equalsIgnoreCase("hoje")) {
				dataConvertida = getData("hoje");
			} else {
				if (value.equalsIgnoreCase("ontem")) {
					dataConvertida = getData("ontem");
				}else{
					if (value.equalsIgnoreCase("amanha")) {
						dataConvertida = getData("amanha");
					}else{
						try {
